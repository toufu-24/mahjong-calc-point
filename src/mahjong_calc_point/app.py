...
def calculate_hand(
    tiles_str, win_tile_str, is_tsumo=False, melds_str="", dora_indicators_str="",
):
    try:
    ...
    if melds_str:
        try:
            for meld_str in melds_str.split(","):
                ...
                if len(parts) == 3:
                    meld = TilesConverter.string_to_136_array(
                        man=parts[0], pin=parts[1], sou=parts[2]
                    )
        ...
        try:
            for part in dora_indicators_str.split(","):
                ...
                if kind == "man":
                    ...
                    dora_tile = TilesConverter.string_to_136_array(honors=tile)[0]
                else:
...
def index():
    ...
    if request.method == "POST":
        ...
        result = calculate_hand(
            tiles_str, win_tile_str, is_tsumo, melds_str, dora_indicators_str
        )
    ...
    return render_template(
        ...
        result=result,
        tiles_str=tiles_str,
        win_tile_str=win_tile_str,
        melds_str=melds_str,
        dora_indicators_str=dora_indicators_str,
        yaku=yaku,
        han=han,
        fu=fu,
        cost=cost,
